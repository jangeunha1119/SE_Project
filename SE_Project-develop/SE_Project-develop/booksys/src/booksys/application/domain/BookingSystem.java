...
//Reservation 클래스 추가
	...
	// noshow, blacklist 추가 -> 노쇼방지 메소드 preventNoShow() 추가
	public void makeReservation(int covers, Date date, Time time, int tno, String name, String phone, int noshow,
			boolean blacklist) {
		if (!doubleBooked(time, tno, null) && !overflow(tno, covers)
				&& !preventNoShow(name, phone, noshow, blacklist)) {
			Booking b = restaurant.makeReservation(covers, date, time, tno, name, phone, noshow, blacklist);
	...
	// noshow 방지하는 기능
	private boolean preventNoShow(String name, String phone, int noshow, boolean blacklist) {
		int NumberNoShow = 0; // NumberNoShow = 노쇼횟수
		boolean whetherBlackList = false; // whetherBlackList = true, 블랙리스트 여부 = true = 블랙리스트 등록
		NumberNoShow = customer.getNoShow();
		whetherBlackList = customer.getBlackList();
		/*
		 * //예약한지 1시간이 지나도 안왔을 경우
		 * 
		 * //Calendar cal = Calendar.getInstance(); //cal.add(Calendar.HOUR, +1);
		 */
		// 도착시간이 예약시간보다 이후인 경우, 노쇼횟수 +1
		if (reservation.getArrivalTime().after(reservation.getTime())) {
			NumberNoShow++;
			customer.setNoShow(NumberNoShow);
		}
		// 노쇼횟수 3회부터는 블랙리스트 등록, whetherBlackList = true이면 블랙리스트
		if (customer.getNoShow() >= 3) {
	...
	// noshow횟수, blacklist여부를 return 하는 get메소드 추가
	public int getNoShow() {
		return customer.getNoShow();
	}

	public boolean getBlackList() {
		return customer.getBlackList();
	}
}
