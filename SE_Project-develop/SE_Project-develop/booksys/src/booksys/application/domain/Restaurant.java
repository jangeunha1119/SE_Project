...
class Restaurant {
	...
	// noshow, blacklist 추가
	Customer getCustomer(String name, String phone, int noshow, boolean blacklist) {
		return cm.getCustomer(name, phone, noshow, blacklist);
	}
	...
	// noshow, blacklist 추가
	public Booking makeReservation(int covers, Date date, Time time, int tno, String name, String phone, int noshow,
			boolean blacklist) {
		Table t = getTable(tno);
		Customer c = getCustomer(name, phone, noshow, blacklist);
